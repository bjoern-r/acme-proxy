...
#
# dns-01 challenge for acme-proxy
# https://github.com/bjoern-r/acme-proxy/

set -e
set -u
set -o pipefail

if [[ -z "${ACMEPROXY_USER}" || -z -z "${ACMEPROXY_KEY}" || -z -z "${ACMEPROXY_URL}" ]] ; then
  echo " - Unable to locate acme-proxy url/user/pw in the environment!  Make sure ACMEPROXY_USER and ACMEPROXY_KEY and ACMEPROXY_URL environment variables are set"
fi

deploy_challenge() {
  local DOMAIN="${1}" TOKEN_FILENAME="${2}" TOKEN_VALUE="${3}"
  echo -n " - Setting TXT record with ${TOKEN_VALUE}"

  curl -s -X POST ${ACMEPROXY_URL}/present \
    -H "X-Api-User: ${ACMEPROXY_USER}" \
    -H "X-Api-Key: ${ACMEPROXY_KEY}" \
    -H "Content-Type: application/json" \
    -d '{"fqdn": "'${DOMAIN}'", "value": "'${TOKEN_VALUE}'"}'
  echo
...
clean_challenge() {
  local DOMAIN="${1}" TOKEN_FILENAME="${2}" TOKEN_VALUE="${3}"
  echo -n " - Removing TXT record from ${DOMAIN}"

  curl -s -X POST ${ACMEPROXY_URL}/cleanup \
    -H "X-Api-User: ${ACMEPROXY_USER}" \
    -H "X-Api-Key: ${ACMEPROXY_KEY}" \
    -H "Content-Type: application/json" \
    -d '{"fqdn": "'${DOMAIN}'"}'
